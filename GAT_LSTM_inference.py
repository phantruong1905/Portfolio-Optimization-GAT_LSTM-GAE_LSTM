...
def split_by_date(df, train_ratio=1, val_ratio=0):
    """
    Split dataframe into train, validation, and test sets based on dates.
...
def create_corr_edges_with_weights(df, threshold):
    """
    Create correlation-based edges between stocks with weights.

    Args:
        df: DataFrame with stock price data
        threshold: Correlation threshold to include an edge (default: 0.0)

    Returns:
        edge_index: Tensor of edge indices [2, num_edges]
        edge_attr: Tensor of edge weights [num_edges]
    ...
    for src in symbols:
        for dst in symbols:
            if src != dst:
                corr_value = corr_matrix.loc[src, dst]
                # Only include edges with correlation above threshold
                if abs(corr_value) >= threshold:
                    i, j = symbol_to_idx[src], symbol_to_idx[dst]
...
def normalize_splits(train_df):
    ...
    # Save raw log_return for target before normalization
    train_df = train_df.copy()
    train_df['log_return_raw'] = train_df['log_return']

    # Infer feature columns (excluding identifiers)
    feature_cols = [col for col in train_df.columns if col not in exclude_cols and col != 'log_return_raw']

    # Fit scaler with DataFrame (not .values) to preserve structure
    ...
    # Debug print to confirm normalization
    print("\nFeatures normalized:")
    print("Feature columns:", feature_cols)
    print("\ntrain_df[feature_cols].head():")
    print(train_df[feature_cols].head())

    return train_df, feature_cols, scaler


def compute_log_return_split(train_df):
    """
    Compute log returns for each split while maintaining the split integrity.
    ...
    full_df = pd.concat([train_df]).sort_values(['Symbol', 'Date'])

    # Compute log returns
    full_df['log_return'] = full_df.groupby('Symbol')['Adj Close'].transform(
...
def create_datasets(train_df, edge_index, edge_attr, feature_cols, seq_len):
    """
    Create GNNLSTMDataset objects for train, validation, and test sets.
...
def prepare_gnn_lstm_data(data_path, seq_len, corr_threshold):
    """
    Complete pipeline to prepare data for GAT-LSTM portfolio optimization.
    ...
    # Split data
    train_df= split_by_date(df)
    print(f"Data split: Train {len(train_df)}rows")

    # Compute log returns
    train_df= compute_log_return_split(train_df)

    # Create correlation-based edges with weights
    edge_index, edge_attr, symbols = create_corr_edges_with_weights(train_df, threshold=corr_threshold)

    # Normalize feature columns
    train_df, feature_cols, scaler = normalize_splits(train_df)

    print(f"Features normalized: {len(feature_cols)} features")

    # Drop unused columns
    train_df = train_df.drop(columns='Adj Close')

    # Create datasets
    train_dataset = create_datasets(
        train_df, edge_index, edge_attr, feature_cols, seq_len
    )

    print(f"Datasets created: Train {len(train_dataset)} sequences")
